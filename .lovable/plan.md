...
- Sweep the arbitrary sizes: replace `text-[9px]`/`text-[10px]` with `t-micro`, `text-[11px]` with `t-label`, `text-xs` with `t-label` or `t-body` depending on role, `text-sm` with `t-body`, `text-base`/`text-lg` with `t-subtitle`/`t-title`. Files with the highest counts first: `invoices.tsx`, `quotations.tsx`, `pipeline.tsx`, `clients.tsx`, `dashboard.tsx`, `app-shell.tsx`, `data-toolbar.tsx`, plus the shared shell and panel components.

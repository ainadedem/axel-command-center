...
function Delta({ value, invert = true }: { value: number; invert?: boolean }) {
  if (value === 0) {
    return <span className="inline-flex items-center gap-0.5 t-label text-muted-foreground"><Minus className="h-3 w-3" /> no change</span>;
  }
  const worse = invert ? value > 0 : value < 0;
  const Icon = value > 0 ? ArrowUpRight : ArrowDownRight;
  return (
    <span className={cn("inline-flex items-center gap-0.5 t-label", worse ? "text-destructive" : "text-success")}>
  ...
  return (
    ...
    >
      <header className="flex items-start justify-between gap-3">
        <div>
          <h2 className="font-display t-subtitle font-semibold">This week at a glance</h2>
          <p className="t-label text-muted-foreground">Last 7 days, compared with the week before.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="rounded-lg border border-border bg-surface-elevated/40 p-3">
          <div className="t-micro uppercase tracking-wider text-muted-foreground">Overdue invoices</div>
          <div className="mt-1 text-xl font-display font-semibold font-tnum">{summary.overdueCount}</div>
          <div className="t-label text-muted-foreground font-tnum">{fmtAmount(summary.overdueExposureMGA, "MGA")} at risk</div>
        </div>
        <div className="rounded-lg border border-border bg-surface-elevated/40 p-3">
          <div className="t-micro uppercase tracking-wider text-muted-foreground">Ladder steps</div>
          <div className="mt-1 text-xl font-display font-semibold font-tnum">
            {summary.stepsLoggedThisWeek}<span className="text-muted-foreground t-body">/{summary.stepsDueThisWeek}</span>
          </div>
          <div className="t-label text-muted-foreground">logged of those that came due</div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <div className="rounded-lg border border-border overflow-hidden">
          <div className="px-3 py-2 t-label uppercase tracking-wider text-muted-foreground border-b border-border">Aging</div>
          {summary.buckets.map((b) => (
            <div key={b.label} className="flex items-center gap-3 px-3 py-2 border-b border-border/40 last:border-0">
              <div className="t-body flex-1">{b.label}</div>
              <div className="t-body font-tnum">{b.count}</div>
              <div className="t-label text-muted-foreground font-tnum w-32 text-right">{fmtAmount(b.exposureMGA, "MGA")}</div>
            </div>
          ))}
        </div>

        <div className="rounded-lg border border-border overflow-hidden">
          <div className="px-3 py-2 t-label uppercase tracking-wider text-muted-foreground border-b border-border flex items-center gap-1.5">
            <Users className="h-3 w-3" /> Who needs to act
          </div>
          {owners.length === 0 ? (
            <div className="px-3 py-4 t-body text-muted-foreground">Nothing outstanding — nobody has open items.</div>
          ) : (
            owners.map((o) => (
              <div key={o.ownerId} className="flex items-center gap-3 px-3 py-2 border-b border-border/40 last:border-0">
                <div className="t-body flex-1 truncate" title={ownerName(o.ownerId)}>{firstName(ownerName(o.ownerId))}</div>
                {o.criticals > 0 && (
                  <span className="t-micro px-1.5 py-0.5 rounded border border-destructive/40 text-destructive bg-destructive/10 uppercase tracking-wider">
                    {o.criticals} critical
                  </span>
                )}
                <div className="t-body font-tnum">{o.count}</div>
                <div className="t-label text-muted-foreground font-tnum w-32 text-right">{fmtAmount(o.exposureMGA, "MGA")}</div>
              </div>
            ))
          )}
        </div>

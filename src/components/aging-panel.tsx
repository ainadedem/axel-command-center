    ...
    <div className="space-y-4">
      <PanelCard bodyClassName="p-0 pb-0 px-0" className="overflow-hidden">
        <div className="px-5 py-2.5 border-b border-border flex items-center justify-between gap-3">
          <div className="text-[11px] uppercase tracking-[0.16em] text-muted-foreground">{tilesTitle}</div>
          {selected && (
            <button
              type="button"
          ...
          {aging.pastDue.map((b) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "p-5 text-left relative transition-colors duration-150 ease-[cubic-bezier(0.2,0,0,1)]",
              ...
              >
                ...
                <div
                  className={cn(
                    "font-display text-2xl font-bold font-tnum mt-2 leading-none",
      ...
      <ChartFrame
        title={title}
        description={description ?? "Open balance by days past due (MGA) — follows the current filters. Click a bar to filter."}
        labelKey="bucket"
        data={chartData}

    ...
    <div className={className}>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            ...
            className="w-full h-10 px-3 rounded-md border border-input bg-background t-body flex items-center justify-between gap-2 hover:border-primary/40 transition"
          >
        ...
        <PopoverContent align="start" className="w-72 p-1">
          <div className="px-2 py-1.5 t-label uppercase tracking-wider text-muted-foreground">
            Up to {MAX_QUOTE_ASSIGNEES} people · {value.length} selected
          </div>
          {loading && <div className="px-2 py-3 t-label text-muted-foreground">Loading people…</div>}
          {!loading && users.length === 0 && (
            <div className="px-2 py-3 t-label text-muted-foreground">
            ...
            {users.map((u) => {
              ...
              return (
                <button
                  ...
                  disabled={!on && atMax}
                  className={cn(
                    "w-full flex items-center gap-2 px-2 py-2 rounded text-left t-body hover:bg-surface-elevated transition disabled:opacity-40 disabled:cursor-not-allowed",
                    on && "bg-primary/5",
                  )}
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block truncate t-label text-muted-foreground">{u.role.replace(/_/g, " ")}</span>
                  </span>
      ...
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-2">
          {selected.map((u) => (
            <span key={u.userId} className="inline-flex items-center gap-1.5 pl-1 pr-2 py-0.5 rounded-full border border-border bg-surface-elevated t-label">
...
export function AssigneeStack({ companyId, ids }: { companyId: string; ids: string[] }) {
  ...
  if (!ids.length) return <span className="text-muted-foreground/50 t-label">—</span>;

...
export const Route = createFileRoute("/.lovable/oauth/consent")({
  ...
  errorComponent: ({ error }) => (
    <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center gap-3 p-6">
      <h1 className="t-title">Authorization request failed</h1>
      <p className="t-body text-muted-foreground">{String((error as Error)?.message ?? error)}</p>
    </main>
  ),
});
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center gap-5 p-6">
      ...
      <div className="rounded-xl border border-border bg-surface-container p-6">
        <h1 className="t-title">Connect {clientName} to AXEL</h1>
        <p className="t-body mt-2 text-muted-foreground">
          {clientName} will be able to read and act on AXEL data that you can access, as you.
        </p>
        {error && (
          <p role="alert" className="t-body mt-3 text-destructive">
            {error}
          </p>
        )}

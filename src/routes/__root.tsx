...
import ogImage from "@/assets/axel-og-white.png.asset.json";
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <h2 className="mt-3 t-title font-medium">Page not found</h2>
...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        <h1 className="t-title font-semibold">Something broke</h1>
        <p className="mt-2 t-body text-muted-foreground">{error.message}</p>
        <button
          onClick={() => { router.invalidate(); reset(); }}
          className="mt-6 inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 t-body font-medium text-primary-foreground hover:opacity-90"
        >
          Try again
        </button>
      </div>
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  ...
  head: () => ({
    ...
    links: [
      ...
      { rel: "stylesheet", href: "https://fonts.googleapis.com/css2?family=Figtree:wght@400;500;600;700;800&family=Inter:wght@400;500;600&family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,400,0,0&display=swap" },
    ],
    scripts: [
      {
